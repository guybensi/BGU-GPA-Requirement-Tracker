...
class CourseTrackerUI:
    ...
    def course_input_form(self):
        ...
        if st.session_state.user_courses:
            ...
            # הוספת עמודת מחיקה
            delete_button_column = st.columns([1, 4])  # עמודה למחיקה
            with delete_button_column[0]:
                delete_button = st.button("🗑️ Delete Selected")
                if delete_button:
                    df.drop(df.index[selected_row], inplace=True)
                    st.session_state.user_courses = df.to_dict(orient="records")
                    st.rerun()

            # אפשרות עריכה בטבלה
            edited_df = st.data_editor(
                df,
                disabled=False,  # לא disable את כל השורות
